...
args['num_classes'] = 93
args["long_tail"] = True #10 classese for imb or 93 classes for long tail
...
'''
args['cut_mix'] = 'CMO_OBJ_DET' # 'CMO' or 'CMO_XAI' or 'CMO_XAI_MASK' or 'CMO_OBJ_DET'
args['sample_method'] = 'freqeuncy' #'topological' or 'freqeuncy' #by defaut, it is frequency
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Move cls_num_list to the appropriate device
    cls_num_list_cuda = torch.from_numpy(np.array(cls_num_list)).float().to(device)
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        # train for one epoch
        train(device, train_loader, model, gc, criterion, optimizer, epoch, args, log_training, tf_writer,weighted_train_loader=weighted_train_loader)

        # evaluate on validation set
        acc1 = validate(device, val_loader, model, criterion, epoch, args, log_testing, tf_writer)
...
def train(device, train_loader, model, gc, criterion, optimizer, epoch, args, log,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args['cut_mix'] == 'CMO' and args['start_cut_mix']< epoch < (
            ...
            output = model(input)
            # Convert lam_list to a PyTorch tensor on the correct device
            lam_tensor = torch.as_tensor(lam_list, dtype=torch.float32, device=device)

            # Compute the weighted loss
            loss = (criterion(output, target) * lam_tensor +criterion(output, target2) * (1. - lam_tensor))
...
def get_targeted_bboxes_with_yolo(images, class_ids, conf_thres=0.15):
    ...
    Args:
        ...
        class_ids (List[int]): List of target class IDs for each image.
...
def validate(device, val_loader, model, criterion, epoch,args, log, tf_writer, flag='val'):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')

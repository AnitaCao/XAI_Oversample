...
x = batch_tensor.cuda()
start = time.time()
saliencys, _ = gc(x, None)

print('Total time: {:.2f} second'.format((time.time()-start)))

for i, saliency in enumerate(saliencys):
    saliency = cv2.resize(np.squeeze(saliency), images[i].size)
    print('Saliency generated by Grad-CAM')
    saliency_visualisation(images[i], saliency)
